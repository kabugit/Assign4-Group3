...
num_rating_count_game <- dat %>%
  ...
  select("Apps" = track_name, "Total rating count" = rating_count_tot)
```
...
It can be seen from Table \@ref(tab:ranknumcount) that the top three apps with the most rating counts are Clash of Clans, Temple Run and Candy Crush Saga. Among them, the number of Clash of Clans and Temple Run rating counts exceeds one million.
...
It can be seen from Figure \@ref(fig:ratingdifference) and Table \@ref(tab:rankratingdifference) that shopping apps have the fastest drop in ratings after last update. Finance and food & drink are followed in the ranking. And only the rating of the shopping apps fell below -0.8.
